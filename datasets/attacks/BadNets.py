...
from .utils import OneClassDataset
...
class PoisonedCIFAR10(CIFAR10):
    ...
    def __init__(self,
        ...
        super(PoisonedCIFAR10, self).__init__(
            ...
            download=False)
        print("CIFAR10")
        add_trigger = AddCIFAR10Trigger(pattern, weight)
        single_source_class = 5  # used when y_target = -2

        assert (y_target >= 0 or y_target in {-2, -1})
        total_num = len(benign_dataset)
        if poisoned_set:
            poisoned_set = f"{poisoned_set}.pt"
        if poisoned_set and Path(poisoned_set).exists():
            self.poisoned_set = torch.load(poisoned_set)  # poisoned indices
        else:
            is_poisonable = (torch.tensor(self.targets) != y_target if y_target >= -1 else
                             torch.tensor(self.targets) == single_source_class)
            poisonable_indices = torch.arange(total_num)[is_poisonable]
            poisoned_num = int(len(poisonable_indices) * poisoned_rate)
            assert poisoned_num >= 0, 'poisoned_num should greater than or equal to zero.'
            self.poisoned_set = poisonable_indices[torch.randperm(len(poisonable_indices))][
                                :poisoned_num]
            if poisoned_set:  # poisoned indices path
        ...
        self.poisoned_transform.transforms.insert(poisoned_transform_index, add_trigger)
        ...
        poisoned_target_transform = (ModifyTarget(y_target) if y_target >= 0 else
                                     ModifyTargetAll2All(num_classes=10) if y_target == -1 else
                                     Remap({single_source_class: 0}))
        self.poisoned_target_transform.transforms.insert(poisoned_target_transform_index,
                                                         poisoned_target_transform)
    ...
    def __getitem__(self, index):
        ...
        if index in self.poisoned_set:  # O(n) operation
...
class BadNets(Base):
    ...
    def __init__(self,
        ...
        if y_target >= -1:
            ...
            self.asr_dataset = OneClassDataset(
                self.poisoned_test_dataset,
                5, # single source class
            )
        else:
            raise ValueError(f"Invalid y_target {y_target}.")
