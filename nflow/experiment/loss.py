...
def loglik_nats_elementwise(model, x):
    """Compute the log-likelihood in nats."""
...
def loglik_bpd_elementwise(model, x):
    """Compute the log-likelihood in bits per dim."""
    return - model.log_prob(x) / (math.log(2) * x[0].shape.numel())
...
def elbo_nats_elementwise(model, x):
    """
    Compute the ELBO in nats.
    Same as .loglik_nats(), but may improve readability.
    """
    return loglik_nats_elementwise(model, x)
...
def elbo_bpd_elementwise(model, x):
    """
    Compute the ELBO in bits per dim.
    Same as .loglik_bpd(), but may improve readability.
    """
    return loglik_bpd_elementwise(model, x)
...
def dataset_elbo_bpd(model, data_loader, device, double=False, verbose=True, return_vals=False, nats_threshold=None):
    with torch.no_grad():
        bpd = 0.0
        count = 0
        vals = []
        ...
        for i, x in enumerate(data_loader):
            if double: x = x.double()
            x = x.to(device)
            val = elbo_nats(model, x).cpu().item()
            if nats_threshold and val < nats_threshold:
                nats_indices.append(i)
            bpd += val * len(x)
            if return_vals: vals.append(val)
            count += len(x)
            if verbose: print('{}/{}'.format(i+1, len(data_loader)), bpd/count, end='\r')
    if return_vals:
        return bpd / count, vals, nats_indices
    else:
        return bpd / count, None, None
